              ...
              <div className="p-12 md:p-16 space-y-16">
                  ...
                  <section className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl space-y-3">
                            <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Conditions</p>
                            <div className="flex flex-wrap gap-2">
                                {profile.preexisting_conditions.length > 0 ? profile.preexisting_conditions.map(c => (
                                    <span key={c} className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[9px] font-bold text-slate-600">● {c}</span>
                                )) : <span className="text-[9px] font-bold text-slate-300 italic">None reported</span>}
                            </div>
                        </div>
                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl space-y-3">
                            <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Critical Allergies</p>
                            <div className="flex flex-wrap gap-2">
                                {profile.known_allergies.length > 0 ? profile.known_allergies.map(a => (
                                    <span key={a} className="px-3 py-1 bg-rose-50 border border-rose-100 text-rose-600 rounded-full text-[9px] font-bold uppercase">● {a}</span>
                                )) : <span className="text-[9px] font-bold text-slate-300 italic">None reported</span>}
                            </div>
                        </div>
                    </div>
                      ...
                      <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-200">
                          <tr>
                            ...
                            <th className="px-6 py-4 text-[8px] font-black uppercase tracking-widest text-slate-500">Active Ingredients</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {medications.map((m, i) => (
                            <tr key={m.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}>
                              ...
                              <td className="px-6 py-5 text-[9px] font-bold text-slate-400 italic leading-relaxed">{m.normalized.active_ingredients.join(', ')}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <section className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-4">
                        <h3 className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-4">Clinical Observations</h3>
                        <div className="space-y-2">
                          {reportData.executive_summary?.smart_advice?.map((a: string, i: number) => (
                            <div key={i} className="text-xs font-bold p-5 bg-white border border-slate-100 rounded-3xl flex gap-4 text-slate-600 shadow-sm transition-all hover:bg-slate-50">
                      ...
                      <div className="space-y-4">
                        <h3 className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-4">Dialogue Targets</h3>
                        <div className="space-y-2">
                    ...
                    <div className="space-y-4">
                      {reportData.safety_flags.map((flag: any, i: number) => (
                        <div key={i} className={`p-10 rounded-[2.5rem] border-2 bg-white flex flex-col md:flex-row items-center gap-10 ${BORDER_COLOR_MAP[flag.risk_color as RiskColor] || 'border-slate-200'}`}>
                          <div className="w-20 h-20 bg-slate-50 rounded-[2rem] flex items-center justify-center text-5xl border border-slate-200">🚨</div>
                          <div className="flex-1 space-y-3 text-center md:text-left">
                            <div className="flex items-center justify-center md:justify-start gap-4">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-rose-600">{flag.flag_type}</h4>
                                <span className={`text-[8px] font-black uppercase px-3 py-1 rounded-full ${COLOR_MAP[flag.risk_color as RiskColor]}`}>Level: {flag.risk_color}</span>
                            </div>
                            <p className="text-base font-bold text-slate-800 italic leading-snug">"{flag.trigger_plain}"</p>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-900 bg-slate-100 px-4 py-1 rounded-full inline-block mt-2">Action: {flag.user_action_plain}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {includeTimeline && checkins.length > 0 && (
                  <section className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="h-72 w-full bg-white rounded-[2.5rem] p-8 border border-slate-200">
